...
import com.example.dongtianming.TPP2.adapters.AutoTextViewAdapter;
import com.example.dongtianming.TPP2.adapters.ListViewAdapter;
import com.example.dongtianming.TPP2.beans.City;
import com.example.dongtianming.TPP2.utils.PinyinUtils;
...
public class CitySearchActivity extends AppCompatActivity implements IndexView.OnIndexChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        search_edittext= (AutoCompleteTextView) findViewById(R.id.search_edittext);
        indexView= (IndexView) findViewById(R.id.indexview);
        searchListView= (ListView) findViewById(R.id.search_listview);
        closeTextView= (TextView) findViewById(R.id.close_textview);
        initData();
        searchListView.setAdapter(new ListViewAdapter(cities,getApplicationContext(),this));
        ...
        indexView.setOnIndexChangeListener(this);//设置变化监听事件

        closeTextView.setOnClickListener(new View.OnClickListener() {//关闭当前Activity
            @Override
        ...
        search_edittext.setAdapter(new AutoTextViewAdapter(cities,this));
        search_edittext.setOnItemClickListener(new AdapterView.OnItemClickListener() {//设置autotextview的每个item的监听
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                TextView textView=view.findViewById(R.id.test555);
                search_edittext.setText(textView.getText().toString());
                String placeName=textView.getText().toString();
                Intent intent=new Intent();
                intent.putExtra("地名",placeName);
                setResult(1,intent);
    ...
    private void updateListView(String word) {
        for (int i=0;i<cities.size();i++){
            String listWord=cities.get(i).getPinyin().substring(0,1);
            if (word.equals(listWord)){
                searchListView.setSelection(i);
                return;
            }
        }
    }
    ...
    public void initData(){
        ...
        for (int i=0;i<cityString.length;i++){
            City city=new City();
            city.setName(cityString[i]);
            city.setPinyin(PinyinUtils.getPinyin(cityString[i]));
            city.setFirst(PinyinUtils.getPinyin(cityString[i]).substring(0,1));
            cities.add(city);
        }
